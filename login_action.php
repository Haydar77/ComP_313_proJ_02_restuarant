...
if ($result->num_rows > 0) {
	...
	if ($result->num_rows > 0) {
		...
		$sql = "UPDATE verification SET status= 0, verification_code='" . $verification_code . "'  WHERE user_id=" . $_SESSION["user_id"];

		//$sql = "UPDATE verification SET status= 0,  WHERE user_id = '". $user_id. "' AND verification_code= '" . $original_verificaton_code . "'";
	  $result = $conn->query($sql);
